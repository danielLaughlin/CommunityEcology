...
#pdf("~/OneDrive - University of Wyoming/Data/Community Ecology/Chapter5/fig5.4.pdf", width=4.1, height=4.1)
par(mfrow=c(2,2), mar=c(3,3,2,2), mgp=c(2,1,0)) # create 2x2 panel

plot(cwm$poros ~ cwm$DAYSUB, pch=19, col="grey50", ylim=c(5,60), cex.lab=0.7, cex.main=0.5, cex=0.01,
     xlab="Flooding duration (days)", ylab="Root aerenchyma (%)", cex.axis=0.5,
     main="a) Community-weighted mean regression
     (supported by 4th corner analysis)")
xvec<-seq(0,250,1)
legend(-5,63,legend=expression(paste("CWM r=0.77, ",italic(p), " < 0.001")),bty="n", cex=0.5)
legend(-5,58,legend=expression(paste("4th corner r=0.67, ",italic(p), " = 0.022")),bty="n", cex=0.5)
...
points(cwm$DAYSUB,cwm$poros,pch=19, col="grey50", cex=0.3)
#axis(1,"Root aerenchyma (%)", line=3 )

plot(cwm$height ~ cwm$DAYSUB, pch=19, col="grey60",ylim=c(0,35), cex.lab=0.7,cex.main=0.5,  cex=0.01, 
     xlab="Flooding duration (days)", ylab="Height (cm)", cex.axis=0.5,
     main="b) Community-weighted mean regression
     (unsupported by 4th corner analysis)")
legend(-5,37,legend=expression(paste("CWM r=0.33, ",italic(p), " < 0.001")),bty="n", cex=0.5)
legend(-5,34,legend=expression(paste("4th corner r=0.26, ",italic(p), " = 0.421")),bty="n", cex=0.5)
#mtext("A", side=3, adj=0, line=0, cex=1.1, font=1)
...
points(cwm$DAYSUB,cwm$height,pch=19, col="grey50", cex=0.3)
...
persp(xy.prob.pred$x, xy.prob.pred$y, xy.prob.pred$z, theta = 30, phi = 20,
      ...
      TxE P < 0.001, Marginal R-square = 0.33", cex.lab=0.7, lwd=0.5, border="grey40", zlim=c(0,1), cex.main=0.5)
...
persp(xy.prob.pred$x, xy.prob.pred$y, xy.prob.pred$z, theta = 30, phi = 20,
      ...
      TxE P = 0.83, Marginal R-square = 0.10", cex.lab=0.7, lwd=0.5, border="grey40", zlim=c(0,1), cex.main=0.5)
...
pdf_species_trait <- lapply(levels(as.factor(sim.trait.data$species)), function(x){
  Mclust(na.omit(sim.trait.data[sim.trait.data$species == x, c("trait")]),warn = TRUE)
})
names(pdf_species_trait) <- levels(as.factor(sim.trait.data$species))
...
#pdf("~/OneDrive - University of Wyoming/Data/Community Ecology/Chapter7/fig7.4_new.pdf", width=4.1, height=6.9)
par(mfrow=c(3,1), mar=c(5,4,2,1))
cols=c("black","grey30","grey50","grey65","grey80")

# T-E
plot(env,t1.pos, pch=16, xlab="Environmental gradient", ylab="Trait value", cex=1, font.lab=1, cex.lab=1.3, ylim=c(-6,6),
     main="a) Trait-environment relationship", cex.main=1.4, cex.axis=0.9)
...
# species traits
tseq <- seq(-6, 6, 0.1)
plot(tseq, dnorm(tseq,-5, 1), type="l", xlab="Trait value", ylab="Probability density", font.lab=1, cex.lab=1.3, col=cols[1],
     lwd=3, main="b) Species trait distributions", cex.main=1.4, cex.axis=0.9, ylim=c(0,0.45))
lines(tseq, dnorm(tseq,-2.5, 1), col=cols[2], lwd=3)
lines(tseq, dnorm(tseq,0, 1), col=cols[3], lwd=3)
lines(tseq, dnorm(tseq,2.5, 1), col=cols[4], lwd=3)
lines(tseq, dnorm(tseq,5, 1), col=cols[5], lwd=3)
legend("top",c("Species A","Species B","Species C", "Species D", "Species E"), bty="n", horiz=TRUE, cex=0.9)

# preds
plot(env, xlim=range(env), ylim=c(0,1), xlab="Environmental gradient", main="c) Traitspace predictions",
     ylab="Predicted relative abundance", cex.lab=1.3, col="white", font.lab=1, cex.main=1.4, cex.axis=0.9)
...
legend("top",c("Species A","Species B","Species C", "Species D", "Species E"),
       col=cols, pch=19, cex=0.9, bty="n", horiz=TRUE)
#dev.off()
### End of Figure 7.4 ###
...
maxent_long <- stack(data.frame(cats.m1[[1]]))
obs_fg_long <- stack(data.frame(ra_fg))
maxent_fg_long <- stack(data.frame(cats.m3[[1]]))

#pdf("~/OneDrive - University of Wyoming/Data/Community Ecology/Chapter8/fig8.2.pdf", width=4.1, height=6.9)
...
#pdf("~/OneDrive - University of Wyoming/Data/Community Ecology/Chapter8/fig8.4.pdf", width=4.1, height=5)
layout(matrix(c(1,2,3,3), 2, 2, byrow = TRUE),
       widths=c(1,1), heights=c(2,2.5))
par(mar=c(3,3,2,2), mgp=c(2,1,0))
plot(obs_long[,1], maxent_long[,1], xlab="Observed relative abundances", ylab="Maximum entropy solutions",
     main="(a) Maximum entropy solutions", pch=19, col="grey50", cex.lab=0.7, cex.main=0.7, cex.axis=0.7, cex=0.7 )
abline(0,1)
text(0.6,0.7,"1:1",cex=0.7)
legend("topleft", expression(paste(R^2 ,"= 0.29")), bty="n")
plot(obs_long[,1], minent_long[,1], xlab="Observed relative abundances", ylab="Minimum entropy solutions",
     main="(b) Minimum entropy solutions", pch=19, col="grey50", cex.lab=0.7, cex.main=0.7, cex.axis=0.7, cex=0.7 )
abline(0,1)
text(0.6,0.7,"1:1",cex=0.7)
legend(0.2,0.3, expression(paste(R^2 ,"= 0.08")), bty="n")

plot(rad_comm_sum, pch=19, ylim=c(0.001, 100), main="(c) Species abundance distributions (summed across all plots)",
     cex.lab=0.9, cex.main=0.7, cex.axis=0.3 )
...
legend("bottomleft", legend=c("Observed relative abundances","Maximum entropy solutions","Minimum entropy solutions"),
       pch=c(19,19,1), col=c(1,"grey70",1), bty="n",cex=0.7)
#dev.off()
